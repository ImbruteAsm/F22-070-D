...
public class Enrichment {
    ...
    private void getServers(String target, DNSHealth dnsHealth) {
        log.info("GETTING THE LISSST");
        String cmdTemplate = "dig +nocmd %s any +noall +answer | awk '{if (NR>3){print}}'| tr -s '\\t' | jq -R 'split(\"\\t\") |{Name:.[0],TTL:.[1],Class:.[2],Type:.[3],IpAddress:.[4]}' | jq --slurp . > %s";
    ...
    public ApplicationSecurity gatherDataApplicationSecurity(String target) throws IOException, InterruptedException {
        ...
        try {
            ...
            issuesN = instance2.scanApplication(target);
            System.out.println(issuesN);
        } catch (Exception e) {
    ...
    public ApplicationSecurity determineRiskNumber(ApplicationSecurity appSecurity) {
        appSecurity.setRiskNumber(this.toInt(appSecurity.getHighSeverityVul().getCsrf()) * Constants.HIGH + this.toInt(appSecurity.getHighSeverityVul().getSqlI()) * Constants.HIGH + this.toInt(appSecurity.getHighSeverityVul().getXss()) * Constants.HIGH + this.toInt(appSecurity.getGeneral().getEnforceHttps()) * Constants.HIGH + this.toInt(appSecurity.getGeneral().getCsp()) * Constants.HIGH + this.toInt(appSecurity.getGeneral().getHsts()) * Constants.MEDIUM + this.toInt(appSecurity.getCookies().getSecure()) * Constants.LOW + this.toInt(appSecurity.getCookies().getHttpOnly()) * Constants.LOW + this.toInt(appSecurity.getGeneral().getCORSPolicy()) * Constants.LOW + this.toInt(appSecurity.getGeneral().getXContentTypeOptions()) * Constants.LOW + this.toInt(appSecurity.getGeneral().getXFrameOptions()) * Constants.LOW);
        return appSecurity;
